--  Arnoldi body — MGS Krylov → upper Hessenberg + optional Jacobi Ritz.

pragma Ada_2022;

with Ada.Numerics.Elementary_Functions;

package body Arnoldi
  with SPARK_Mode => Off
is
      ...
      for I in 1 .. N loop
         for J in 1 .. N loop
            if I = J then
               ...
               A (I, J) := 0.1 / Float (J - I + 1);
            else
               A (I, J) := 0.05 / Float (I - J + 1);
            end if;
         end loop;
      end loop;
      return A;
   ...
   begin
      case Kind is
         ...
         when Nonsymmetric_DD =>
            return Make_Nonsymmetric_DD (N);
      end case;
   end Make_Example;
   ...
   -------------------------------------------------------------------------
   -- Inlined Jacobi diagonalization (educational; for nearly-symmetric H)
   -------------------------------------------------------------------------
   ...
   begin
      ...
      for J in 1 .. M_Want loop
         ...
         if Nrm <= Params.Tol then
            ...
            --  h_{j+1,j} ≈ 0; do not store a further subdiagonal entry.
            Res.Stat := Breakdown;
            Res.Success := True;
            for Col in 1 .. J loop
               for Row in 1 .. J loop
                  Res.H (Row, Col) := Hloc (Row, Col);
               end loop;
            end loop;
         ...
         if J < M_Want then
            --  Store subdiagonal h_{j+1,j} inside the m×m Hessenberg.
            Hloc (J + 1, J) := Nrm;
            for I in 1 .. N loop
               Vj (I) := W (I) / Nrm;
            end loop;
            ...
            --  Finished requested steps; leftover h_{m+1,m} in H_Extra only
            --  (outside the m×m block) unless Steps = n (full Krylov).
            null;
         end if;
      end loop;
      ...
      for Col in 1 .. Res.Steps loop
         for Row in 1 .. Res.Steps loop
            Res.H (Row, Col) := Hloc (Row, Col);
         end loop;
      end loop;
      --  When we completed M_Want < n, H_Extra = ‖w‖ after last step is
      --  h_{m+1,m}; when M_Want = n the Krylov is full and residual is
      --  typically tiny — still report the measured norm.
      if Params.Keep_V then
         for Col in 1 .. Res.Steps loop
            for Row in 1 .. N loop
               Res.V (Row, Col) := Basis (Row, Col);
   ...
   begin
      ...
      begin
         ...
         --  Educational path: extract real Ritz when H is nearly
         --  symmetric (typical for symmetric A → H ≈ tridiagonal).
         --  Float Arnoldi leaves O(1e-5)–O(1e-4) asymmetry; symmetrize
         --  before Jacobi when Asym ≤ Ritz_Sym_Tol.
         Sym_Ok := Asym <= 1.0E-3;
         if not Sym_Ok then
            --  Leave Hessenberg exposed; mark success without Ritz.
            return Res;
         end if;

         --  Symmetrize for a stable Jacobi sketch.
         for I in 1 .. M loop
            for J in I + 1 .. M loop
               declare
                  S : constant Float := 0.5 * (T (I, J) + T (J, I));
               begin
                  T (I, J) := S;
                  T (J, I) := S;
               end;
   ...
   function Run
     (A      : Matrix;
      ...
      Params : Parameters := Default_Parameters) return Result
   is
      Built : constant Result := Build_Hessenberg (A, V0, Params);
   begin
